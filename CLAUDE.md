...
- **Patient Pages project `drkyana`** bindings must be named **`DB`, `KV`** (code reads those) + secrets `ANTHROPIC_API_KEY`, `PATIENT_AGENT_TOKEN`, `IP_HASH_SALT`, build var `VITE_PATIENT_AGENT_TOKEN` (= the token), and (for urgent-notify) the `EMAIL` send binding + `RECEPTIONIST_FROM`/`DR_KYANA_NOTIFY_EMAIL` vars.
