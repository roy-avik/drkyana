...
- **Ops Worker `drkyana-ops`** (`apps/ops`): hosts the scheduled jobs the OpenNext admin worker can't (it exports only `fetch`). Three Cloudflare **Workflows**, each fired by a `schedules` cron on its binding (no `scheduled()` handler needed): `ReminderWorkflow` (03:00 UTC → `runReminders`, the appointment+urgent digest to Dr Kyana), `RetentionWorkflow` (03:30 → `runRetention`, purge spent OTPs + compact idle session transcripts, PDPA), `ScheduledResearchWorkflow` (04:00 → `runScheduledResearch`). Deploy **manually**: `cd apps/ops && npm run deploy` (NOT built by the Pages Git integration). Secrets on this Worker: `ANTHROPIC_API_KEY` (research workflow), `OPS_TRIGGER_TOKEN` (bearer for the `POST /trigger/<wf>` manual test fire); `IP_HASH_SALT` is NOT needed — no ops code path reads it. Deploy needs **wrangler >= 4.113** (older releases send a Workflow-create payload the API rejects with code 10002 when `schedules` is present). `GET /` returns health + each job's last-run record (kept in KV `ops:last:<name>`). The admin `/api/cron/*` routes remain the on-demand triggers.
